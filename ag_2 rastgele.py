...
weights_input_hidden = np.array([[1, 0.5, -0.3, 1], [-0.5, -0.2, 0.7, 0.3], [2, 0.8, 0, -0.1]])
weights_hidden_output = np.array([[1, 0.2], [0.9, 0.3], [0.7, -0.5], [0.3, 0.7]])
...
for i in range(input_size):
    G.add_node(f'Input {i + 1}', value=input_layer[i])  


for i in range(hidden_size):
    G.add_node(f'Hidden {i + 1}', value=hidden_layer[i]) 


for i in range(output_size):
    G.add_node(f'Output {i + 1}', value=output_layer[i])  
...
nx.draw(G, pos, with_labels=True, labels=node_labels, node_size=700, node_color='skyblue', font_size=8, font_color='black', font_weight='bold', arrowsize=20)

for (i, j), label in edge_labels.items():
    x, y = (pos[i][0] * 0.25 + pos[j][0] * 0.75, pos[i][1] * 0.25 + pos[j][1] * 0.75)
    plt.text(x, y, label, color='red', fontsize=8, ha='center', va='center')
